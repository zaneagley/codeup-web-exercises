    ...
    function weatherApp(data) {
        ...
        arrDays.forEach(function (data, i) {
            ...
            if (document.getElementById("temperature").checked === true) {
                output += '<p class="temperatureInfo m-0">' + Math.round(((data.main.temp_min - 273.15) * 1.8) + 32);
                output += '°/' + Math.round(((data.main.temp_max - 273.15) * 1.8) + 32) + '°</p>';
            }
    ...
    function weatherMarkerInfo(data) {
        var output = '';
        if (document.getElementById("temperature").checked === true) {
            output += '<p class="temperatureInfo m-0">' + Math.round(((data.list[0].main.temp_min - 273.15) * 1.8) + 32);
        ...
        if (document.getElementById("humidity").checked === true) {
            output += '<br><p class="humidityInfo m-0"><strong>Humidity</strong>: ' + data.list[0].main.humidity + '</p>';
        }
        if (document.getElementById("wind").checked === true) {
            output += '<br><p class="windInfo m-0"> <strong>Wind</strong>: ' + data.list[0].wind.speed + '</p>';
        }
        if (document.getElementById("pressure").checked === true) {
            output += '<br><p class="pressureInfo m-0"><strong>Pressure</strong> ' + data.list[0].main.pressure + '</p>';
        }
        return output
    }
    //
    ...
    $('#temperature').change(function () {
        $('.temperatureInfo, .iconInfo').toggleClass('hideTemp');
    });
